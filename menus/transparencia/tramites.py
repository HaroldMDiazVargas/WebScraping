...
# 32 Trámites y servicios
class TramitesServicios(abstractBase.Requisites):
    def checkRequisites(self, soup, browser, newUrl):
        ...
        elif(soup.select_one(".tramite-row")):
            date = getTextData(soup.select_one(".tramite-row").select_one(".tramite__date"))
            title = getTextData(soup.select_one(".tramite-row").select_one(".tramite__name"))
            info = getTextData(soup.select_one(".tramite-row").select_one(".tramite__description"))
            onPage = True
